...
if st.button("Forecast AQI"):
    ...
    try:
        ...
        # Match Selected Date
        result = forecast[
            forecast["ds"].dt.date == selected_date
        ]
        ...
        if len(result) > 0:
            ...
            st.success(
                f"Predicted AQI for {city} on {selected_date}: {predicted_aqi:.0f}"
            )
            ...
            st.warning(
                "Selected date is outside forecast range."
            )
